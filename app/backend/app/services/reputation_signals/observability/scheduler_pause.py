...
def _log(message: str) -> None:
    try:
        ...
        print(f"[REPUTATION][SCHEDULER_PAUSE] log failed: {exc}")


def pause_scheduler(reason: str = "reputation_signals") -> None:
...
def resume_scheduler(reason: str = "reputation_signals") -> None:
    global _PAUSE_COUNT, _REASON, _COOLDOWN_UNTIL
    with _LOCK:
        _PAUSE_COUNT = max(0, _PAUSE_COUNT - 1)
        if _PAUSE_COUNT == 0:
            _REASON = ""
            cooldown_seconds = float(os.getenv("REPUTATION_MONITOR_RESUME_COOLDOWN_SECONDS", "120"))
...
def pause_status() -> dict:
    with _LOCK:
        ...
        return {
            ...
            "source": "reputation_signals",
        }


@contextmanager
def scheduler_pause(reason: str = "reputation_signals") -> Iterator[None]:
